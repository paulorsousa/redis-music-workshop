...
export default function SimilarSongs({ songId }) {
  ...
  return (
    <Section title="Similar Songs" loading={loading} error={error} elapsed={elapsed} onRefresh={refresh}>
      {similar.length === 0 ? (
        <p style={{ color: '#b3b3b3', fontStyle: 'italic' }}>
          🚀 No similar songs available yet — complete Module 6 to enable this feature!
        </p>
      ) : (
        <ul className="song-list">
          {similar.map((song, i) => (
            <li key={song.id || i}>
              ...
              {song.score != null && (
                <span style={{ color: '#1db954', fontSize: '0.85rem' }}>({song.score.toFixed(2)})</span>
              )}
            </li>
          ))}
        </ul>
